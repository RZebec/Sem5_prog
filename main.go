...
func main() {
	...
	indexPageHandler := webui.IndexPageHandler{SessionManager: &sessionManager, Config: config}
	http.HandleFunc("/", indexPageHandler.ServeHTTP)

	filesHandler := files.FilesHandler{}
	http.HandleFunc("/files/", filesHandler.ServeHTTP)

	loginPageHandler := login.LoginPageHandler{SessionManager: &sessionManager, Config: config}
	http.HandleFunc("/login", loginPageHandler.ServeHTTP)

	loginHandler := login.LoginHandler{UserManager: &sessionManager, Config: config}
	http.HandleFunc("/user_login", loginHandler.ServeHTTP)

	logoutHandler := logout.LogoutHandler{UserManager: &sessionManager, Config: config}
	logoutWrapper := wrappers.AuthenticationHandler{Next: logoutHandler, Config: config, SessionManager: &sessionManager}

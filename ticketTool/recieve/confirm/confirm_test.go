...
func TestGetAllAcknowledges(t *testing.T) {
	...
	testee := Confirmator{}
	acknowledge := testee.GetAllAcknowledges(mails)
...
func TestGetSingleAcknowledges(t *testing.T) {
	...
	testee := Confirmator{}
	acknowledges := testee.GetAllAcknowledges(mails)
	/*
		selected acknowledge should be not anymore in the List of acknowledges
	*/
	acknowledgeList, selectedAck := testee.GetSingleAcknowledges(acknowledges, "testSubject0")
	...
	*/
	equalAcknowledgeList := testee.GetAllAcknowledges(mails)
	acknowledgeList, selectedAck = testee.GetSingleAcknowledges(acknowledges, "testSubject3")
...
func TestDeleteFromArray(t *testing.T) {
	...
	newAcknowledges := testee.deleteFromArray(acknowledges, acknowledges[0])

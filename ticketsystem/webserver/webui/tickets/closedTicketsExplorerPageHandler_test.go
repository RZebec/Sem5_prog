...
*/
func TestClosedTicketsExplorerPageHandler_ServeHTTP_ValidRequest(t *testing.T) {
	...
	testee := ClosedTicketsExplorerPageHandler{TicketContext: mockedTicketContext, TemplateManager: mockedTemplateManager,
		Logger: testhelpers.GetTestLogger()}

	// Only tickets with state closed should be shown.
	testTickets := getTestTickets()
	var expectedTicketData []ticketData.TicketInfo
	for _, possibleTicket := range testTickets {
		if possibleTicket.State == ticketData.Closed {
			expectedTicketData = append(expectedTicketData, possibleTicket)
		}
	}
	expectedPageData := closedTicketsExplorerPageData{
	...
	req, err := http.NewRequest("GET", "/closed_tickets", nil)
	if err != nil {
		t.Fatal(err)
	}
...
*/
func TestClosedTicketsExplorerPageHandler_ServeHTTP_WrongRequestMethod(t *testing.T) {
	mockedTicketContext := new(mockedForTests.MockedTicketContext)
	mockedTemplateManager := new(templateManager.MockedTemplateManager)
	...
	req, err := http.NewRequest("POST", "/closed_tickets", nil)
	if err != nil {
		t.Fatal(err)
	}
...
*/
func TestClosedTicketsExplorerPageHandler_ServeHTTP_ContextError_RenderError(t *testing.T) {
	mockedTicketContext := new(mockedForTests.MockedTicketContext)
	mockedTemplateManager := new(templateManager.MockedTemplateManager)
